...
public class S_ModuleSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            LoadStage(0);
            currentStage.stagePrefabs.Insert(0, fillerModule);
        }
    ...
    //         for (int x = -renderDistanceSide; x <= renderDistanceSide; x++)
    //         {
    //             var gridPos = new Vector2Int(playerGridPos.x + x, playerGridPos.y + z);
    //
    //             var stageToUse = PlayerPrefs.GetInt(TutorialKey) == 0 ? tutorialStages : currentStage;
    //             var prefabIndex = playerGridPos.y % stageToUse.stagePrefabs.Count;
    //             var modulePrefab = stageToUse.stagePrefabs[prefabIndex];
    //
    //             if (!moduleGridPositions.Contains(gridPos))
    //             {
    //                 SpawnModuleAt(modulePrefab, gridPos);
    //             }
    //         }
    //     }
    //
